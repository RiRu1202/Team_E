...
public class Player_Stop_Controller_t : MonoBehaviour
{
    private CameraController_t cameraController_t;
    ...
    void Start()
    {
        ...
        cameraController_t = FindObjectOfType<CameraController_t>();
    }
    ...
    private IEnumerator HandleWallCollision()
    {
        ...
        if (cameraController_t != null)
        {
            Debug.Log("カメラ停止（0.3秒間）");
            cameraController_t.isPaused = true;

            yield return new WaitForSeconds(0.3f);

            cameraController_t.isPaused = false;
            Debug.Log("カメラ再開！");
        }
    }
}
